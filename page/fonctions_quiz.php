...
function select_full_quiz($db, $id)
{
	if($id != 0)
	{
		$query = "SELECT quiz.id as id, quiz.titre as titre, quiz.id_membre as id_membre, quiz.publie as publie, ".
				"quiz_question.id as idd_question, quiz_question.numero_question as numero_question, quiz_question.text as question_texte, quiz_question.multi as multi, quiz_question.temps_reponse as temps_reponse, quiz_reponse.id_question as id_question, ". 
						"quiz_reponse.id as id_reponse, quiz_reponse.numero_reponse as numero_reponse, quiz_reponse.text as reponse_texte, quiz_reponse.juste as juste ".
				"FROM quiz, quiz_question, quiz_reponse ". 
				"WHERE quiz.id = 73 AND quiz.id = quiz_question.id_quiz ".
				"AND quiz.id = quiz_reponse.id_quiz ".
				"AND quiz_reponse.id_question = quiz_question.numero_question ;";
	}
	...
	while ($row = mysqli_fetch_assoc($result)) 
	{
		...
		{
			$quiz["id"] = $row["id"];
			$quiz["titre"] = iconv("ISO-8859-1", "UTF-8//IGNORE", $row['titre']);
			$quiz["id_membre"] = $row["id_membre"];
			$quiz["publie"] = $row["publie"];
		}
		/*** QUESTION ***/
		$quiz["question"][$row["numero_question"]]["idd_question"]= $row['idd_question']; // id_question
		$quiz["question"][$row["numero_question"]]["numero_question"]= $row['numero_question']; // numero_question
		$quiz["question"][$row["numero_question"]]["question_texte"]= iconv("ISO-8859-1", "UTF-8//IGNORE", $row['question_texte']); // question texte
		$quiz["question"][$row["numero_question"]]["temps_reponse"]= $row["temps_reponse"]; // temps rep
		$quiz["question"][$row["numero_question"]]["multi"]= $row["multi"]; // multi
		/*** REPONSE ***/
		$quiz["question"][$row["numero_question"]]["reponses"][$row["numero_reponse"]]["id_reponse"] = $row['id_reponse']; //id reponse
		$quiz["question"][$row["numero_question"]]["reponses"][$row["numero_reponse"]]["numero_reponse"] = $row['numero_reponse']; //num reponse
		$quiz["question"][$row["numero_question"]]["reponses"][$row["numero_reponse"]]["reponse_texte"] = iconv("ISO-8859-1", "UTF-8//IGNORE", $row['reponse_texte']); //reponse a la question
		$quiz["question"][$row["numero_question"]]["reponses"][$row["numero_reponse"]]["juste"] = $row['juste']; // juste ou non
		
		$res[count($res)] = $row;
		$res[count($res) - 1]['question_texte'] = iconv("ISO-8859-1", "UTF-8//IGNORE", $row['question_texte']);
		$res[count($res) - 1]['reponse_texte'] = iconv("ISO-8859-1", "UTF-8//IGNORE", $row['reponse_texte']);
		$res[count($res) - 1]['titre'] = iconv("ISO-8859-1", "UTF-8//IGNORE", $row['titre']);
    } 
	
	return $quiz;
}

/*************************** SELECT FULL QUIZ CASCADE ************************************/

function select_full_quiz_cascade($db, $id)
{
	if($id !== 0)
	{
...
function modif_question($db, $id, $numero_question, $text, $multi, $temps_reponse)
{
	if($numero_question !== "" && $text !== "" && $multi !== "" && $temps_reponse !== "")
	{
		$query = "UPDATE quiz_question 	
				SET numero_question = '".$numero_question."', text = '".$text."', multi = ".$multi.", temps_reponse = ".$temps_reponse." ";
	}
	else
	{
		return "nok";
...
function select_reponse($db, $id_quiz, $id_question, $id_reponse)
{
	if($id_quiz !== "" && $id_question !== "" && $id_reponse !== "")
	{
	...
	$query .= " WHERE id_quiz = ".$id_quiz." AND id_question = ".$id_quiz." AND id_reponse = ".$id_reponse." ";
	$result = mysqli_query($db, $query);
	$result = mysqli_fetch_row ($result);
...
{
	if($id_quiz !== "" && $text !== "" && $juste !== NULL )
	{
		$query = "UPDATE quiz_reponse
				SET numero_reponse = '".$numero_reponse."', text = '".$text."', juste = '".$juste."' ";
	}
	else
	{
		return "nok";
...
function insert_question($db, $id_quiz, $numero_question, $text, $multi, $temps_reponse)
{
	$query = "INSERT INTO `quiz_question` (`id_quiz`, `numero_question`, `text`, `multi`, `temps_reponse`) 
			VALUES ('".$id_quiz."', '".$numero_question."', '".$text."', '".$multi."', '".$temps_reponse."' )";
...
function insert_reponse($db, $id_quiz, $id_question, $numero_reponse, $text, $juste)
{
	$query = "INSERT INTO `quiz_reponse` (`id_quiz`, `id_question`, `numero_reponse`, `text`, `juste`) 
			VALUES ('" .$id_quiz . "', '"  . $id_question . "', '" . $numero_reponse . "', '" . $text . "', " . $juste . ")";
	$result= mysqli_query($db, $query);
